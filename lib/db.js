...
//   - no jobCoords (city-only, no postcode/street) → neutral, NO distance shown
//   - distance ≤ max → good
//   - distance > max → bad (with distance)
// Returns { verdict: "good"|"bad"|"neutral", reason: string, distanceKm: number|null }
export function evaluatePreference(workplaceType, city, jobCoords, homeCoords, maxDistanceKm) {
