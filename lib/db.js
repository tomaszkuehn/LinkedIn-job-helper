...
// extensions send Origin automatically. We keep requests minimal (one shot
// on save) and cache the result in storage so we don't hit the service on
// every page load.
...
// Logic:
//   - remote → good
//   - no homeCoords → neutral (feature disabled)
//   - no workplaceType → neutral
//   - city in CITY_COORDS AND distance ≤ max → good
//   - city in CITY_COORDS AND distance > max → bad (with distance)
//   - city not in CITY_COORDS → neutral (can't measure, no false red)
// Returns { verdict: "good"|"bad"|"neutral", reason: string, distanceKm: number|null }
export function evaluatePreference(workplaceType, city, homeCoords, maxDistanceKm) {
