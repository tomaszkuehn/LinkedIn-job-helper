...
export function distanceFromHome(city, homeCity) {
...
// Logic:
//   - remote → good
//   - no homeCity configured → neutral (feature disabled)
//   - city detected AND distance ≤ maxDistanceKm → good (same agglomeration)
//   - city detected AND distance > maxDistanceKm → bad (with distance shown)
//   - city not detected or not in CITY_COORDS → neutral (can't measure)
// Returns { verdict: "good"|"bad"|"neutral", reason: string, distanceKm: number|null }
export function evaluatePreference(workplaceType, city, homeCity, maxDistanceKm) {
  ...
  if (d === null) {
    // City not in coords table — can't measure. Treat as neutral so we don't
    // false-positive red-banner jobs in small towns we simply don't know.
